...
Timepicker.prototype = {
    ...
    _generateHtml: function ()
    {
        ...
        html += '<div class="ui-datepicker-title" style="margin:0; display:none;">';
		
        html += '<span class="fragHours">08</span><span class="delim">:</span><span class="fragMinutes">45</span></div>';
		
        html += '<div class="ui-datepicker-visible-title" style="padding:5px; text-align:center;">';
        html += '<span id="fragHoursVis" class="fragHoursVis">08</span>:';
        html += '<span id="fragMinutesVis" class="fragMinutesVis">45</span> ';
        html += '<span id="fragAmPmVis" class="fragAmPmVis">AM</span>';
    ...
    _writeTime: function (type, fragment)
    {
		...
		if (type == 'hour') {
            $('#' + this._mainDivId + ' span.fragHours').text('');
        ...
        if (type == 'minute') {
			...
			$('#fragMinutesVis').text(fragment);
        }
		
		//Formatting time in 12-hour format
		var AmPm = 'AM';
        if (type == 'hour') {
			if (oldFrag > 11) AmPm = 'PM';
			if (oldFrag > 12) oldFrag = oldFrag - 12;
			if (oldFrag == 0) oldFrag = 12;
            if (oldFrag < 10) oldFrag = '0' + oldFrag;
            $('#fragHoursVis').text(oldFrag);
            $('#fragAmPmVis').text(AmPm);
        }
