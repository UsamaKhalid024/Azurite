...
(function($) {
	...
	$.widget("ui.sfFilters", {
		...
		_sort : function(p_array_of_objects, p_property_name) {
			...
			var buf;
			for (var i=0; i<p_array_of_objects.length; i++) {
				for (var j=0; j<p_array_of_objects.length-1; j++) {
					if (p_array_of_objects[j][p_property_name] > p_array_of_objects[j+1][p_property_name]) {
						buf = p_array_of_objects[j];
						p_array_of_objects[j] = p_array_of_objects[j+1];
						p_array_of_objects[j+1] = buf;
					}
				}
			}
		},
		...
		_addButton : function() {
			...
			this.element.append('<div id="sf_filter_add_place" class="sf_filter_add_place" style="width:30px; margin-top: 10px; margin-left:'+ (margin - 30) +'px;"><input type="button" id="sf_filter_add_btn" value="add search criteria" /></div>');
		...
		_FieldChange : function(p_element) {
			...
			if(field_type == "DATE") {
				...
				$("#sf_value_" + filter_index).after('<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" id="sf_filter_clear_btn_'+ filter_index +'"  value="clear" />');
		...
		_addFilter : function(p_filter_object, p_is_load) {
			...
			if(p_filter_object.name == "") {
				try {
					field_name = this.options.fieldset[0].value.toLowerCase();
				} catch(err) {
					this.log(err);
				}
			}else {
				field_name = p_filter_object.name.toLowerCase();
			}
			...
			html += '&nbsp;';
			html += this._generateSelect('sf_filter_field', this.filter_index, this.options.fieldset, p_filter_object.name.toLowerCase(), 150);
			html += '&nbsp;';
			...
			if(current_field_type == "DATE") {
				...
				$("#sf_value_" + this.filter_index).after('<input type="button" class="ui-button ui-widget ui-state-default ui-corner-all" id="sf_filter_clear_btn_'+ this.filter_index +'"  value="clear" />');
		...
		_generateTextControl : function(p_filter_index, p_value) {
			...
			var html = '<input  type="text" id="sf_value_' + p_filter_index + '" value="' + _htmlspecialchars(value) +'" class="ext ui-widget-content ui-corner-all" style="width:180px;">';
			return html;
		},
		...
		_generatePicklistControl : function(p_filter_index, p_value, p_field_name, p_widget) {
			...
			html += '<span id="sf_filter_lookup">';
				html += '<span class="lookupInput ' + p_widget.options.lookupAddClass + '">';
					html += '<input type="text" size="20" name="sf_value_' + p_filter_index + '" maxlength="255" id="sf_value_' + p_filter_index + '" value="' + _htmlspecialchars(p_value) + '"/>';
					html += '<a title="Lookup (New Window)" onclick="setLastMousePosition(event)" ';
					html += 'href="'
					html += "javascript:filterLookupValueElem='sf_value_" + p_filter_index +"'; openPopupFocus('"+p_widget.options.lookupURL+"?event=load&target="+p_widget.options.goalTargetID+"&picklist=" + p_field_name.replace('slca2__', '') + "', 'filter_lookup', 620, 430, 'width=620,height=430,resizable=yes,toolbar=no,status=no,scrollbars=yes,menubar=no,directories=no,location=no,dependant=no', false, true);";
					html += '">';
		...
		_generateLookupControl : function(p_filter_index, p_value, p_field_name, p_widget) {
			...
			html += '<span id="sf_filter_lookup">';
				...
				if (p_field_name == 'OwnerId' && (objName == 'case' || objName == 'lead' || objName == 'contract' || postfix == '__c')) {
					html += '<span class="lookupInput ' + p_widget.options.lookupAddClass + '">';
						html += '<input type="text" size="20" name="' + id + '" maxlength="255" id="' + id + '" value="' + _htmlspecialchars(p_value) + '" />';
